...
import { 
  ...
  Globe, Printer, Image as ImageIcon, Zap, ArrowRight 
} from 'lucide-react';
...
export default function Navbar({ categories }: { categories: string[] }) {
  ...
  useEffect(() => {
    ...
    const unsubCart = onSnapshot(q, (snapshot) => {
        ...
        <div className="flex items-center gap-5 text-white">
          ...
          {!user ? (
            <div className="hidden md:flex items-center gap-4">
              ...
              <Link href="/register" className="bg-[#2563EB] px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl">Sign Up</Link>
            </div>
          ) : (
            <Link href="/profile" className="flex items-center gap-4 pl-4 border-l border-white/10 group">
              <div className="hidden sm:flex flex-col items-end">
                <p className="text-[9px] font-black uppercase tracking-widest text-white group-hover:text-blue-400 transition-colors">
                  {profileData?.name || "User"}
                </p>
                <p className="text-[7px] font-bold text-zinc-500 uppercase tracking-widest">₹{profileData?.walletBalance || 0}</p>
              </div>
              <div className="w-10 h-10 rounded-full border-2 border-blue-500 p-0.5 overflow-hidden transition-transform group-hover:scale-105">
                 <img 
                   src={profileData?.profileImage || `https://ui-avatars.com/api/?name=${profileData?.name || "User"}`} 
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-[200] lg:hidden">
          ...
          <div className="absolute left-0 top-0 h-full w-[85%] max-w-sm bg-[#0F172A] border-r border-white/5 p-6 shadow-2xl flex flex-col animate-in slide-in-from-left duration-500 overflow-hidden">
            ...
            <div className="flex flex-col gap-1 overflow-y-auto no-scrollbar pb-10 text-white">
              {/* Navigation Options */}
              ...
              <div className="py-4 border-b border-white/5">
                <p className="text-zinc-500 text-[9px] font-black uppercase tracking-widest mb-4">Our Services</p>
                <div className="grid gap-3">
                  <Link href="/services/stocks" className="flex items-center gap-4 p-3 bg-white/5 rounded-2xl" onClick={closeMenu}><ImageIcon size={18} className="text-blue-500"/> <span className="text-[10px] font-bold uppercase">Stock Media</span></Link>
                  <Link href="/services/development" className="flex items-center gap-4 p-3 bg-white/5 rounded-2xl" onClick={closeMenu}><Globe size={18} className="text-amber-500"/> <span className="text-[10px] font-bold uppercase">Web & App Dev</span></Link>
                  <Link href="/services/printing" className="flex items-center gap-4 p-3 bg-white/5 rounded-2xl" onClick={closeMenu}><Printer size={18} className="text-emerald-500"/> <span className="text-[10px] font-bold uppercase">Printing Hub</span></Link>
                </div>
              </div>
              ...
              <div className="mt-6">
                {!user ? (
                  ...
                  <Link href="/profile" className="flex items-center justify-between p-5 bg-zinc-900 border border-white/5 rounded-[2rem] hover:bg-zinc-800 transition-colors" onClick={closeMenu}>
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-full border border-blue-500 p-0.5 overflow-hidden">
                        <img 
                          src={profileData?.profileImage || `https://ui-avatars.com/api/?name=${profileData?.name || "User"}`} 
                          className="w-full h-full rounded-full object-cover" 
                          alt="Profile" 
                      ...
                      <div className="flex flex-col">
                        <span className="text-[10px] font-black uppercase tracking-widest text-white">{profileData?.name || "User"}</span>
                        <span className="text-[7px] font-bold text-zinc-500 uppercase tracking-widest">₹{profileData?.walletBalance || 0}</span>
                      </div>
                    </div>
